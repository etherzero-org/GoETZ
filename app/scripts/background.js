 ...
 * @property {boolean} isRevealingSeedWords - True if GoETZ words are currently being recovered, and should be shown to user.
 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows GoETZ to use custom provider strategies for known networks.
 ...
 * @property {number} conversionRate - A number representing the current exchange rate from the user's preferred currency to ETZ.
 ...
 * @property {Array} lostAccounts - TODO: Remove this feature. A leftover from the version-3 migration where our GoETZ-phrase library changed to fix a bug where some accounts were mis-generated, but we recovered the old accounts as "lost" instead of losing them.
 * @property {boolean} forgottenPassword - Returns true if the user has initiated the password recovery screen, is recovering from GoETZ phrase.
 */

/**
 * @typedef VersionedData
 * @property {MetaMaskState} data - The data emitted from GoETZ controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the GoETZ controller, and sets up all platform configuration.
...
async function initialize () {
  ...
  log.debug('GoETZ initialization complete.')
}
 ...
 * @returns {Promise<MetaMaskState>} Last data emitted from previous instance of GoETZ.
 */
async function loadStateFromPersistence () {
  ...
  if (versionedData && !versionedData.data) {
    ...
    if (diskStoreState && diskStoreState.data) {
      ...
      raven.captureMessage('GoETZ - Empty vault found - recovered from diskStore', {
      ...
      raven.captureMessage('GoETZ - Empty vault found - unable to recover')
    }
  }
  ...
  if (!versionedData) {
    throw new Error('GoETZ - migrator returned undefined')
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('GoETZ - Localstore not supported')
    })
  }
...
/**
 * Initializes the GoETZ Controller with any initial state and default language.
...
function setupController (initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('GoETZ - Persistence pipeline failed', error)
    }
  )
  ...
  function persistData (state) {
    if (!state) {
      throw new Error('GoETZ - updated state is missing', state)
    }
    if (!state.data) {
      throw new Error('GoETZ - updated state does not have data', state)
    }
    if (localStore.isSupported) {
  ...
  /**
   * Connects a Port to the GoETZ controller via a multiplexed duplex stream.
   * This method identifies trusted (GoETZ) interfaces, and connects them differently from untrusted (web pages).
  ...
  function connectRemote (remotePort) {
    ...
    if (isMetaMaskInternalProcess) {
      ...
      controller.setupTrustedCommunication(portStream, 'GoETZ')
...
// On first install, open a window to GoETZ website to how-it-works.
